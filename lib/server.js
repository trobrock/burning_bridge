...
(function() {
  ...
  Server = (function() {
    ...
    Server.prototype.pong = function(client) {
      return client.send("" + this.hostname + " PONG " + this.hostname + " :" + this.hostname);
    };

    Server.prototype.welcome = function(client, user) {
      this.message(client, "welcome", "" + (user.nick()) + " Welcome " + (user.mask()));
      this.message(client, "yourHost", "" + (user.nick()) + " Your host");
      this.message(client, "created", "" + (user.nick()) + " This server was created");
      return this.message(client, "myInfo", "" + (user.nick()) + " myIrcServer 0.0.1");
    };

    Server.prototype.motd = function(client, user) {
      this.message(client, "motdStart", "" + (user.nick()) + " :- Message of the Day -");
      this.message(client, "motd", "" + (user.nick()) + " myIrcServer 0.0.1");
      return this.message(client, "motdEnd", "" + (user.nick()) + " :End of /MOTD command.");
    };

    Server.prototype.list = function(client, user) {
      var room, _i, _len, _ref;
      this.message(client, "listStart", "" + (user.nick()) + " Channel :Users  Name");
      _ref = this.rooms.rooms;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        room = _ref[_i];
        this.message(client, "list", "" + (user.nick()) + " " + (room.name.snakeCase()) + " " + room.users.length + " :[]");
      }
      return this.message(client, "listEnd", "" + (user.nick()) + " :End of /LIST");
    };
  ...
  handler = function(socket) {
    ...
    carrier.carry(socket, function(line) {
      var api_token, channel, command, message, r, room, subdomain, target, user, users, _i, _len, _ref, _ref1;
